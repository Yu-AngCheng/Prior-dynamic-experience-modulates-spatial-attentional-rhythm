...
figure(30)
subplot(1,2,1);
PSD_even_diff = normalize(PSD_post_even,'norm',1) - normalize(PSD_pretest_even,'norm',1);
n_perms = 100000;
...
subplot(1,2,2);
PSD_odd_diff = normalize(PSD_post_odd,'norm',1) - normalize(PSD_pretest_odd,'norm',1);
n_perms = 100000;
[datobs, datrnd] = cluster_test_helper(PSD_odd_diff,n_perms,'t'); % randomize data at each time-point
[h, p, clusterinfo] = cluster_test(datobs,datrnd,1,0.05,0.05,'sum');
fprintf('Out of %d tests, %d is significant. \n',length(h),sum(h));
