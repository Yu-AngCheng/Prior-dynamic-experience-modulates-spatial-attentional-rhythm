...
for sub = 1:size(width_pretest, 3)
    ...
    xx = pretest_sub(:,4) == 0;pretest_sub(xx,4) = 2; % re-code
...
for sub = 1:size(width_pretest, 3)
    ...
    PSD_pretest(:,sub) = Myfft(detrend(C_IC_Y,detrendnumber),hann(length(C_IC_Y)),N,fs);
end
...
text(2.6,4.6e-3,'*','FontWeight','bold','HorizontalAlignment','center');
ylim([0,0.005])
